...
모바일에서 자기갱신·위험도구 승인·이미지 양방향. 파이프 복원 근본원인 수정 동반. (/update CC 버전업 주입은 출시 직후 폐기 — 15.2)
...
### 15.2 `/update` — CC 버전업 주입 (폐기, v0.3.2)

`!claude update` 주입으로 CC 버전업을 터미널 직접 없이 폰에서 실행. 그러나 `claude update` silent-fail 잦음(GitHub #5494) + 사실상 쓸모없음 판단으로 **v0.3.2 출시 직후 폐기**. `update_cc_command.py` 삭제, 봇 메뉴·HELP·문서·테스트 정리. CC 버전업 필요 시 터미널 직접 `npm i -g @anthropic-ai/claude-code@latest`. 구 설계는 git 히스토리 참조.
...
## 16. v0.3.3 — 보드 재설계 + 라이프사이클 근본 + `/update-adhd` 2단계 (2026-07-07)

모바일 탭 위주 UX. 번호 버튼-only 보드 → 기능버튼 우선 + 인라인 slot 팝업. `/close` WT 탭 종료 근본 수정. `/update-adhd` 사전 확인 인라인 Yes/No.

### 16.1 보드 재설계 — 기능버튼 우선 + 인라인 slot picker

**배경**: 12.x의 ReplyKeyboard = 번호 버튼만(3열×2행 `1️⃣.`~`6️⃣.`). 대표님 요구:
> "open, close, pin, doctor, help, use, update-adhd 등 **기능 위주**로 바꾸고, close·use 같은 **번호 필요 버튼은 인라인**으로 하나 떠서 폰에서 탭만으로"

= 기능버튼 메인, 번호(주입용) 보조 키패드, `/close /stop /use /new`는 버튼 탭 시 **인라인 번호 팝업**.

**설계**:
- `pin_board.keyboard_markup()` 재작성 — 기능버튼 4행(📋/list 📌/pin 🆕/new · 📂/open ✖️/close ⏹/stop · 🎯/use ❓/help · 🩺/doctor 🔄/update-adhd) + 번호 키패드 2행(1️⃣–6️⃣, 주입용). 버튼 텍스트 = `이모지 /명령` → 탭하면 그 텍스트 그대로 전송 → 라우터 매칭. 번호 버튼은 순수 이모지(InjectCommand 매칭, 본문 없음 = pending 토글, 기존 12.x 흐름 보존).
- `commands/base.py::normalize_command()` 신규 헬퍼 — 선행 이모지/공백/ZWJ strip 후 lower. 각 Command.`match()`가 `(msg.text).strip().lower()` 대신 적용 → `✖️ /close` 같은 이모지+명령 버튼 매칭. InjectCommand는 번호 이모지 매칭이라 normalize 미적용(영향 0).
- `core/slot_picker.py` 신규 — `build_slot_keyboard(active_nums, action, sticky_num)`(callback_data `s:<action>:<num>`, 활성 슬롯만) + `send_picker(tg, chat, action, reg, sticky)`. 활성 0→"열린 터미널 없음" 답장, 1→즉시실행(팝업 생략), 2+→인라인 팝업 송신 후 msg_id 반환.
- `close/stop/use/new_command` — 인자 없으면 `slot_picker.send_picker()` 분기(기존 "사용법: …" 답장 대체).
- `router._handle_callback` — `p:`/`a:` 앞에 `s:` 분기 추가. 가상 Message(`/close <num>`) 생성 → 해당 Command.handle 재진입(명령 코드 중복 0). 보안(allowed chat)은 `s:` 진입 전 이미 통과.
- `/update`(15.2 이미 폐기) 보드/메뉴에서도 완전 제거.

### 16.2 `/close` WT 탭 종료 — find_tab_root + terminate_tree

**배경**: 기존 4.6b `/close` = `WM_CLOSE` PostMessage + `taskkill /F /PID /T`. WT 기본 `closeOnExit:"graceful"`은 정상 종료만 탭 닫음 → `taskkill /F`(강제)를 비정상으로 취급 → **탭 안 닫힘**(대표님 실측, #42).

**수정**:
- `core/proc_win.py::find_tab_root(cc_pid)` — CC 부모체인을 `WindowsTerminal.exe` 직계 자식(= 탭 루트 `cmd.exe`)까지 역추적, 그 pid 반환.
- `close_command` → `terminate_tree(tab_root_pid)` — 탭 트리 전체 종료.
- WT `profiles.defaults.closeOnExit:"always"` — 강제 종료여도 탭 닫음(4.6b graceful 전제 설계 대체, 사용자 로컬 WT settings).

### 16.3 `/update-adhd` 2단계 — 인라인 Yes/No

**배경**: 기존 15.1 = 즉시 pull→pytest→restart. 변경 없어도 강제 실행 + 변경사항 사전 미확인.

**수정** (`update_adhd_command.py`):
1. 현재(`__version__`) vs 최신(`git rev-list --left-right --count`) + 최신 CHANGELOG 섹션 발췌 → 답장.
2. 인라인 Yes/No 버튼(`u:update:yes|no`).
3. router `u:` callback → Yes 시 기존 pull→pytest→detached restart(15.1 로직 재사용). No/timeout → 취소 답장.

### 16.4 검증

- pytest **333 passed**(회귀 없음 — normalize가 슬래시 앞 이모지 없으면 no-op라 기존 테스트 안전).
- 라이브: 보드 기능버튼 4행 렌더 / `✖️ /close` 버튼 탭→인라인 slot 팝업→번호 탭→종료 / `/close` WT 탭 닫힘 / `/update-adhd` Yes/No 팝업 — 대표님 실측 2026-07-07.

---

## 17. v0.3.4 — 간헐 주입 실패 수정 (2026-07-07)

### 17.1 `_write_record` 청크 분할 — bracketed-paste 감지 회피

**배경(#38)**: pipe_win 주입이 **간헐** 실패 — 텍스트가 CC 입력창에만 남고 제출 안 됨. 대표님 1번 세션(백호)에서 **중간 길이** 메시지로 간헐 재현(이미지+캡션 오해 → 실은 간헐 주입 누락).

**근본원인**: `host.py::_write_record`가 본문을 **통째 PTY write** → CC TUI가 빠른 연속 바이트를 **bracketed-paste**로 감지 → 끝 `\r`이 제출(Enter)이 아닌 **줄바꿈**으로 처리 → 텍스트 입력창 잔류. v0.3.3의 "본문·`\r` 분리 write + sleep"은 중간 길이에서 잔존.

**수정** (`host.py::_write_record` v2):
- 본문을 **8자 청크**로 분할, 청크마다 **15ms sleep**(사람 타이핑 흉내) → paste 감지 회피.
- 본문 후 **80ms sleep** → 단독 `\r`(submit).
- 단문(8자 이하) = 청크 1개 = 거의 무해.
- 부작용: 본문 길이에 비례한 주입 지연(100자 ≈ 200ms).

**검증**: `tests/test_host_write_record.py` 5종(청크분할+`\r` / 청크간 sleep / dead-pty skip / invalid-utf8 replace / 단문 단일청크). 라이브: 리스폰된 백호(1번)에서 중간 길이 주입 정상 실측(대표님 확인 2026-07-07).


---

## 18. v0.3.5 — 진행 보드 + 무음 알림 + perm-hook 강화 (2026-07-07)

작업중 슬롯 실시간 가시성 + 알림음 제거 + 승인 게이트 입력 로그 난독화.

### 18.1 ProgressBoard — 슬롯별 작업진행 카운터

**배경**: 상태 보드(12.x)는 ⭕📝❌ 마크만. "지금 몇 초째 돌고 있나" 실시간 카운터 부재. 대표님 요구로 busy 슬롯별 live 카운터 추가.

**설계** (`imadhd/boards/progress_board.py`, 신규):
- router가 1초마다 `reg.active()` 폴링 → `ProgressBoard.sync(sessions)`.
- busy 슬롯 = `🟡 N번 작업중 (Xs)` 메시지. 최초 `send`로 생성, 이후 `edit_message_text`로 초수 갱신(1s).
- busy → idle 전환 시 `delete_message`(카운터 자동 제거).
- **완료 결과는 별도 reply DM**(reply_hook). 보드는 "작업중" 상태만 표현(대표님 지시 2026-07-07 — 완료 DM과 중복 표기 방지).
- **토글**: `IMADHD_PROGRESS_BOARD=0`(env, config.py override=True 로드) → 보드 생성 스킵(`router.py` L129 `if os.environ.get(...) != "0"`). 기본 활성.

커밋: `aa7ba22`.

### 18.2 무음 전송 — disable_notification

**배경(대표님 정정 경위)**: "작업중 알람 안 울리게" → 1차 "소리만 안 나게"(disable_notification) → "팝업도 안 뜨게" 재요청 → env OFF(메시지 자체 제거) 오버적용 → "DM은 와야지" 최종 정정 → **DM은 오되 소리만 안 남**으로 확정.

**설계** (`progress_board._ensure`):
- 최초 `send` 호출에 `disable_notification=True`(`client.send` 이미 파라미터 지원, L150).
- `edit_message_text`/`delete_message`는 본래 알림음 없음 → 최초 send만 mute.
- **정직 한계(보고)**: `disable_notification=True` = Telegram silent = **소리 확실 억제**. 화면 배너/헤드업 팝업은 Telegram 클라이언트 OS 알림 설정 영역이라 봇이 100% 통제 못 함 → 배너까지 잡으려면 대표님이 Telegram 해당 채팅 알림설정 "사운드 없음" 처리(클라이언트 단).

커밋: `4a4c48a`.

### 18.3 perm-hook 입력 로그/승인 본문 강화 (동작 변화 없음)

**배경**: `perm_hook` debug 로그가 주입 본문을 `body={inject_text!r}`로 평문 기록(PII/명령 노출), 승인 본문에 사용자 command를 `<code>...</code>`에 raw 삽입(`parse_mode=HTML` 깨짐/인젝션 위험).

**수정** (`hooks/perm_hook.py`):
- `_input_fingerprint(text)` — `len=N sha256=<12hex>` 형태. debug 로그 `body={...!r}` → `{_input_fingerprint(inject_text)}`. 원문 미기록.
- `build_approval_body(prefix, tool_name, summary)` — `html.escape(summary, quote=False)` 후 `<code>`로 감쌈. `<`/`>`/`&` 이스케이프.
- timeout 거부 `emit_deny(reason)` 헬퍼로 통일.
- **송신 실패 경로는 fail-open 유지**(L157-159 변경 없음 — 막지 않음).
- verdict 로직(allow/deny/timeout) 변경 없음.

**검증**: `tests/test_perm_hook.py`에 `build_approval_body` import + `test_approval_body_escapes_html` 추가(`<script>` → `&lt;script&gt;`, `&` → `&amp;`). pytest 335 passed.
