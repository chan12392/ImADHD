...
## Status
`v0.3.9` — **cross-platform** (Windows `pipe_win` default + `sendkeys_win` fallback; Linux `tmux_linux`). Single-machine (router + terminals on the same host).

### What's new
- **`v0.3.9`** — **Linux parity**: `/stop` now works on Linux (was Windows-only — `TmuxLinuxTransport.send_key` sends `C-c`); tmux session name is configurable via `IMADHD_TMUX_PREFIX` (default `claude`, neutral — no more hardcoded session-name prefix); `IMADHD_TRANSPORT` unset now **auto-detects** by platform (`nt`→`sendkeys_win`, POSIX→`tmux_linux`) so a bare install on Linux no longer defaults to the Windows transport; the Linux `step1_pm2` installer step now also launches `imadhd-watchdog` (runtime 2nd-line defense, was Windows-only); `/close` on a legacy/resume slot with no `tmux_pane` now falls back to the prefix session and kills it (was a zombie-session leak); per-pane inject locking (was a single global lock that serialized all sessions). No Windows code changes.
- **`v0.3.8`** — **image routing popup for bare photos**: a photo sent to the bot with no number/sticky/pending and no unambiguous active target used to be silently dropped — now it pops an inline "↘️ which terminal?" button row; tap → the image is routed there (10 min TTL).
...
| `IMADHD_TRANSPORT` | no | input transport — `pipe_win` (Windows, focus-less, **recommended**), `sendkeys_win` (Windows, focus-stealing fallback), `tmux_linux` (Linux). **Auto-detected by platform when unset** (`nt`→`sendkeys_win`, POSIX→`tmux_linux`); `install` also records the detected value in `.env`. The `.env` value is authoritative and overrides any ambient env var. |
| `IMADHD_TMUX_PREFIX` | no | Linux only: prefix for tmux session names created by `/open` and the fallback target for injection/`/close`/`/stop` when a slot has no `tmux_pane` (default `claude`). Set to match an existing tmux session name to target it. |
...
- **Linux** — input is `tmux send-keys` into the pane captured at `SessionStart`. Each Claude Code session runs in its own tmux session/pane; the registry tracks `tmux_pane` to target the right one. `IMADHD_TRANSPORT` is auto-detected as `tmux_linux` on POSIX when unset. `/open` creates sessions named `<IMADHD_TMUX_PREFIX>-<unixtime>` (default prefix `claude`); `/stop` sends `C-c` to abort a run; `/close` kills the session (falling back to the prefix session for legacy slots without a `tmux_pane`). The installer also launches `imadhd-watchdog` (heartbeat-stale respawn) on Linux, matching Windows.
